...
public sealed class DvripClient : IDisposable
{
        ...
        IProgress<long>? progress = null, CancellationToken ct = default)
    {
        ...
        return await ConvertVideoAsync(destinationPath, outputFormat, ct);
    }
    ...
    private static async Task<string> ConvertVideoAsync(string rawPath, string outputFormat, CancellationToken ct)
    {
        ...
        var ffmpegArgs = format == "mp4"
            ? $"-y -i \"{rawPath}\" -c:v libx264 -preset fast -crf 23 -c:a aac -movflags +faststart \"{outputPath}\""
            : $"-y -i \"{rawPath}\" -c:v libx264 -crf 23 -c:a copy \"{outputPath}\"";
